...
exit 2 原生 BLOCK 信号 + 双通道 API 错误恢复 + /goal 混合守护。BLOCK 使用 exit code 2 + stderr（不参与 JSON 校验，与 CC 原生 /goal 评估器并行不受 JSON validation failed 影响）。StopFailure 通道无条件恢复 CC-level API 错误（socket/429/503）。Stop Phase 0 覆盖消息级错误（~85 模式 / 5 源检测）。DeepSeek V4 thinking mode 兼容。语言无关，零外部依赖。纯 Python 标准库。
...
此外，**API 错误**（socket 断开、HTTP 429/502/503/500、限流、过载、认证失败等）会中断**任何**任务——无论是否 /goal 模式。hello-goal 通过**双通道**覆盖：

- **StopFailure 事件**：CC v2.1.78+ 在 API 错误时触发。`_goal_failure.py` 用 `exit 2 + stderr` 无条件 BLOCK 恢复。
- **Stop Phase 0**：消息级 API 错误（~85 模式 / 5 源检测）。仅匹配短消息（<100 字符）和 system transcript，避免元讨论误触发。

**hello-goal v2.3.10** 的 BLOCK 使用 **exit code 2 + stderr**（CC 原生 block 信号），不参与 JSON 校验。与 CC 原生 /goal 评估器（DeepSeek 等第三方 LLM 可能输出非 JSON）并行运行时，原生评估器的 `JSON validation failed` 不会连带丢弃 hello-goal 的 BLOCK。LLM 语义分析自动适配 DeepSeek V4 thinking mode。API 不可用状态缓存（120 秒 TTL）。
...
| 场景 | 无 hello-goal | 有 hello-goal v2.3.10 |
|------|-------------|------------------|
...
所有 BLOCK 决策使用 **exit code 2 + stderr**（CC 原生 block 信号，不参与 JSON 校验）。PASS 决策使用 exit 0 + 空 JSON `{}`。LLM 语义分析结果仅决定走哪个分支。
...
```
hooks/hooks.json
├── Stop (command, 30s)          ← 核心守护：四层级联分析 → exit 2
├── StopFailure (command, 3s)    ← API 错误恢复：CC-level 错误 → exit 2
├── SessionStart (command, 5s)   ← 清理过期状态，初始化会话
└── PostCompact (command, 3s)    ← 压缩后刷新检测缓存

scripts/_goal_guard.py (~850 行, 零依赖)
├── handle_stop()           ← Phase 0-3 主逻辑
├── _detect_api_error()     ← API 错误 5 源检测（~85 模式 + 状态缓存）
├── _detect_goal_active()   ← 三级 /goal 检测（原生标记 + 命令 + summary）
├── _structural_score()     ← 行为结构信号加权（4 信号）
├── _llm_check()            ← LLM 语义分析（DeepSeek V4 thinking=disabled）
├── _is_api_available()     ← API 可达性检查（120 秒缓存）
├── _block()                ← exit 2 + stderr（CC 原生 block 信号）
└── _pass()                 ← exit 0 + 空 JSON {}

scripts/_goal_failure.py (18 行, 零依赖)
└── StopFailure 守护        ← exit 2 + stderr 无条件 BLOCK
```
...
| `plugins/hello-goal/.claude-plugin/plugin.json` | 插件元数据（v2.3.10） |
...
**A:** 两者作为独立的 stop hook 并行运行。当原生评估器输出非 JSON 文本时，CC 报告 "JSON validation failed"。hello-goal v2.3.10 通过 **exit 2 + stderr** 解决此冲突：BLOCK 使用 CC 原生 exit code 2 信号（不参与 JSON 校验），原生评估器的 JSON 失败不会连带丢弃 hello-goal 的 BLOCK。同时 StopFailure 通道处理 CC-level API 错误。
</details>

<details>
<summary><strong>Q: 为什么用 exit 2 而不是 JSON 输出 BLOCK？</strong></summary>

**A:** CC 原生 /goal 评估器（使用 DeepSeek 等第三方 LLM）可能输出非 JSON 文本，导致整个 Stop hook 批次报 `JSON validation failed`。hello-goal 改用 exit code 2 + stderr（CC 原生的 "阻止停止" 信号），不需要 JSON 解析，与原生评估器的 JSON 失败完全隔离。
</details>

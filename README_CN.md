...
| 场景 | 无 hello-goal | 有 hello-goal v2.3.3 |
|------|-------------|------------------|
...
| `plugins/hello-goal/.claude-plugin/plugin.json` | 插件元数据（v2.3.3） |
...
**A:** 两者作为独立的 stop hook 并行运行。hello-goal v2.3.3 使用 `print()` 走 `sys.stdout` 确保 CC 独立捕获其硬编码 JSON 输出。当原生评估器输出非 JSON 文本（使用 DeepSeek 等第三方大模型时常见）时，hello-goal 的 JSON 输出仍保持有效并可独立校验。但需注意 CC 内部的 hook 批量处理机制可能导致原生评估器的 JSON 校验错误影响整批 hook —— 这是 CC 层面的架构限制。
</details>

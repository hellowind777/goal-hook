...
混合守护插件 —— `/goal` 任务自动监控，防止非正常终止。行为结构分析 + LLM 语义兜底。语言无关，零提示词侵入。
...
**goal-hook v2.0** 以单一 command-type Stop hook 实现三层级联守护，自动检测并阻止上述非正常终止，让 `/goal` 循环持续到任务真正完成。
...
| 场景 | 无 goal-hook | 有 goal-hook v2.0 |
|------|-------------|------------------|
...
| 文件 | 用途 |
|------|------|
| `plugins/goal-hook/hooks/hooks.json` | 三钩子注册（Stop + SessionStart + PostCompact） |
| `plugins/goal-hook/scripts/_goal_guard.py` | 混合守护主脚本 |
| `plugins/goal-hook/.claude-plugin/plugin.json` | 插件元数据（v2.0.1） |
...
**A:** 不会。三层分析都有 PASS 判断。任务正常完成时，行为信号低于阈值且 LLM 确认真完成，hook 放行。
</details>
